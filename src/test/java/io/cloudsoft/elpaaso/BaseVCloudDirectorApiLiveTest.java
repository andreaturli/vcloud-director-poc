...
public abstract class BaseVCloudDirectorApiLiveTest extends BaseContextLiveTest<VCloudDirectorContext> {
    ...
    protected void initTestParametersFromPropertiesOrLazyDiscover() {
        ...
        org = context
                .getApi()
                .getOrgApi()
                .get(find(context.getApi().getOrgApi().list(),
        ...
        if (any(Lists.newArrayList(vAppTemplateUrn, networkUrn, vdcUrn), Predicates.isNull())) {
            ...
            if (networkUrn == null) {
                Optional<Network> optionalNetwork = tryFindNetworkInOrgWithFenceMode(Network.FenceMode.BRIDGED);
            ...
            if (networkUrn == null) {
                Optional<Network> optionalNetwork = tryFindNetworkInOrgWithFenceMode(Network.FenceMode.NAT_ROUTED);
                if (optionalNetwork.isPresent()) {
                    network = optionalNetwork.get();
                    networkUrn = network.getId();
                }
    ...
    public Optional<VAppTemplate> tryFindVAppTemplateInOrg() {
        ...
        if (optionalVAppTemplate.isPresent()) {
            ...
            Logger.CONSOLE.warn("%s doesn't own any vApp Template in org %s; vApp templates: %s", context.getApi()
                    .getCurrentSession().getUser(), org.getName(),
                    Iterables.transform(vAppTemplates, prettyVAppTemplate));
        }
    ...
    public Optional<Vm> tryFindVmInOrg() {
        FluentIterable<Vm> vms = FluentIterable.from(vdc.getResourceEntities())
                .filter(ReferencePredicates.<Reference> typeEquals(VM)).transform(new Function<Reference, Vm>() {
        ...
        if (optionalVm.isPresent()) {
            ...
            Logger.CONSOLE.warn("%s doesn't have any vm in org %s; vms: %s", context.getApi().getCurrentSession()
                    .getUser(), org.getName(), Iterables.transform(vms, prettyVm));
        }
    ...
    public Optional<Network> tryFindNetworkInOrgWithFenceMode(final Network.FenceMode fenceMode) {
        ...
        Optional<Network> optionalNetwork = tryFind(networks, new Predicate<Network>() {
            ...
            public boolean apply(Network input) {
                if (input.getConfiguration().getFenceMode().equals(fenceMode)) {
    ...
    public FluentIterable<Media> findAllEmptyMediaInOrg() {
        ...
        return FluentIterable.from(vdc.getResourceEntities()).filter(ReferencePredicates.<Reference> typeEquals(MEDIA))
                .transform(new Function<Reference, Media>() {

                    @Override
                    public Media apply(Reference in) {
                        return context.getApi().getMediaApi().get(in.getHref());
                    }
    ...
    protected void cleanUpVApp(VApp vApp) {
        ...
        // Wait for busy tasks to complete (don't care if it's failed or
        // successful)
        // Otherwise, get error on remove "entity is busy completing an
        // operation.
    ...
    protected VAppNetworkConfiguration getVAppNetworkConfig(VApp vApp) {
        if(vApp == null)
            return null;
        Set<VAppNetworkConfiguration> vAppNetworkConfigs = vAppApi.getNetworkConfigSection(vApp.getHref())
                .getNetworkConfigs();
     ...
     protected Set<VAppNetworkConfiguration> listVappNetworkConfigurations(VApp vApp) {
         return vAppApi.getNetworkConfigSection(vApp.getHref()).getNetworkConfigs();
      }
